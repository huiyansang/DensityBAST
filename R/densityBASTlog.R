#' Perform BASTION Density Estimation - Log-Linear model
#'
...
#'
#' @examples
densityBASTlog = function(learners,
                       MCMC,
                       BURNIN,
                       THIN,
                       shape_hp = 0.5,
  ...
  lambda_output = vector("list", (MCMC-BURNIN)/THIN)
  membership_output = vector("list", (MCMC-BURNIN)/THIN)
  if(detailed_output) {
    acceptance_output = vector("list", (MCMC-BURNIN)/THIN)
    log_marginal_output = vector("list", (MCMC-BURNIN)/THIN)
  }
  if(graph_detailed_output) {
    graph_output = vector("list", (MCMC-BURNIN)/THIN)
  }
  ...
  for(learner in 1:N_LEARNERS) {
    ...
    if(normalized) {
      polygon_areas[[learner]] = polygon_areas[[learner]]/original_area
    }
    ...
    lambda_values[[learner]] = generateLambdaVals(FALSE,
                                                  k,
                                                  shape_hp,
                                                  rate_hp,
                                                  cluster_ppp[[learner]],
  ...
  for(iter in 1:MCMC) {
    ...
    for(learner in sampling_order) {
      # Calculate thinning probabilities (based on multiplicative relationship)
      log_rho_s = numeric(N)
      for(i in 1:N_LEARNERS) {
        log_rho_s = log_rho_s + log(polygon_constants[[i]][data_polygon_id[[i]]])
      }
      log_delta_s = log((polygon_constants[[learner]][data_polygon_id[[learner]]])) - log_rho_s
      delta_s = exp(log_delta_s)
      point_included = purrr::rbernoulli(N, p = delta_s)
      # Thinned data (for M-H proposal)
    ...
    if(iter > BURNIN & (iter - BURNIN) %% THIN == 0) {
      for(learner in 1:N_LEARNERS) {
        if(normalized) {
          lambda_output[[(iter-BURNIN)/THIN]][[learner]] = lambda_values[[learner]]/original_area
        } else {
          lambda_output[[(iter-BURNIN)/THIN]][[learner]] = lambda_values[[learner]]
        }
        membership_output[[(iter-BURNIN)/THIN]][[learner]] = learner_components[[learner]]$membership
        if(detailed_output) {
          acceptance_output[[(iter-BURNIN)/THIN]][[learner]] = acceptance_prob[learner]
          log_marginal_output[[(iter-BURNIN)/THIN]][[learner]] = log_Lik[learner]
        }
        if(graph_detailed_output) {
          graph_output[[(iter-BURNIN)/THIN]][[learner]] = current_graphs[[learner]]
        }
      }
    }
  }
