...
#' @param eval_lpp A spatstat.linnet::lpp object whose linear network is equivalent to `as.linnet(densityBASTOutput$data)`
...
#'
#' @return A matrix with dimension (number of evaluation points) x (number of posterior samples); each row corresponds to a grid point and each column is a posterior sample for that point.
#' @export
#'
#' @examples

getLinearDensitySamples = function(densityBASTOutput, eval_lpp, intensity = TRUE, log_linear = FALSE) {
  ...
  n_grid_points = nobjects(eval_lpp)
  ...
  for(learner in 1:n_learners) {
    grid_polygon_membership[[learner]] = spatstat.linnet::as.linfun.lintess(meshes[[learner]])(eval_lpp)
  }
